...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="row">
        <div class="col-12 d-flex justify-content-between">
            <div class="breadcrumb-list">
                <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Itineraries /</span> List</h4>
            </div>
            <div class="butns">
                <a href="{{url('admin/itineraries/create')}}" class="btn btn-primary ms-2">Add New</a>
                <!-- <a href="{{url('admin/itineraries/sync')}}" class="btn btn-success">Sync</a> -->
                <a href="{{url('admin/itineraries/trashed')}}" class="btn btn-danger ms-2"ms-2>Trashed</a>
    ...
    <!-- Responsive Table -->
    <div class="card">
        <h5 class="card-header">Itineraries</h5>
        <div class="card-body">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="d-flex justify-content-end">
                        <a class="btn btn-sm btn-primary" href="{{asset('assets/files/itineraries-sample.csv')}}" download="itineraries-sample.csv">Download Sample</a>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '#check-all', function() {
            $('.itinerary-checkbox').each(function() {
                $(this).prop('checked', $(this).is(':checked')? false : true);
                $('#delete-selected-itineraries').addClass('disabled');
                $('.itinerary-checkbox:checked').each(function() {
                    $('#delete-selected-itineraries').removeClass('disabled');
                });
            });
        });

        $(document).on('click', '.itinerary-checkbox', function() {
        ...
        $(document).on('click', '#delete-selected-itineraries', function () {
            var selectedItineraries = [];
            $('.itinerary-checkbox:checked').each(function() {
                selectedItineraries.push($(this).attr('data-itinerary-id'));
            });
            if (selectedItineraries.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{route('delete_multiple_itineraries')}}",
                            method: 'POST',
                            data: {
                                _token: "{{csrf_token()}}",
                                data_ids: selectedItineraries 
                            },
                            success: function(response) {
                                if (response.status) {
                                    ...
                                    Swal.fire(
                                        'Failed!',
                                        'Failed to delete selected itineraries.',
                                        'error'
                                    );
                                }
                            }
                ...
                Swal.fire(
                    ...
                    'Please select at least one itinerary to delete.',
                    'info'
                );
            }
        });
        ...
        $(document).on('click', '.delete-itinerary', function () {
            var selectedItinerary = $(this).attr('data-itinerary-id');
            if (selectedItinerary.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{route('admin.itineraries.destroy')}}",
                            method: 'POST',
                            data: {
                                _token: "{{csrf_token()}}",
                                data_id: selectedItinerary 
                            },
                            success: function(response) {
                                if (response.status) {
                                    Swal.fire(
