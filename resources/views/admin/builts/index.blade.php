...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="row">
        <div class="col-12 d-flex justify-content-between">
            ...
            <div class="butns">
                <a href="{{url('admin/builts/create')}}" class="btn btn-primary ms-2">Add New</a>
                <!-- <a href="{{url('admin/builts/sync')}}" class="btn btn-success">Sync</a> -->
                <a href="{{url('admin/builts/trashed')}}" class="btn btn-danger ms-2"ms-2>Trashed</a>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="d-flex justify-content-end">
                        <a class="btn btn-sm btn-primary" href="{{asset('assets/files/built-sample.csv')}}" download="built-sample.csv">Download Sample</a>
                    </div>
...
order: [],
        ...
        $(document).on('click', '#check-all', function() {
            $('.user-checkbox').each(function() {
                ...
                $('#delete-selected-builts').addClass('disabled');
                $('.user-checkbox:checked').each(function() {
                    $('#delete-selected-builts').removeClass('disabled');
                });
            });
        });
        ...
        $(document).on('click', '#delete-selected-builts', function () {
            var selectedBuilts = [];
            $('.user-checkbox:checked').each(function() {
                selectedBuilts.push($(this).attr('data-user-id'));
            });
            if (selectedBuilts.length > 0) {
                Swal.fire({
                    title: 'Are you sure?',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{route('delete_multiple_builts')}}",
                            method: 'POST',
                            data: {
                                _token: "{{csrf_token()}}",
                                user_ids: selectedBuilts 
                            },
                            success: function(response) {
                                if (response.status) {
                                    ...
                                    Swal.fire(
                                        'Failed!',
                                        'Failed to delete selected builts.',
                                        'error'
                                    );
                                }
                            }
        ...
        $(document).on('click', '.delete-user', function () {
            ...
            if (selectedUser.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{route('admin.builts.destroy')}}",
                            method: 'POST',
                            data: {
                                _token: "{{csrf_token()}}",
