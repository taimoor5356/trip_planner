...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="row">
        <div class="col-12 d-flex justify-content-between">
            <div class="breadcrumb-list">
                <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Seasons /</span> List</h4>
            </div>
            <div class="butns">
                <a href="{{url('admin/seasons/create')}}" class="btn btn-primary ms-2">Add New</a>
                <!-- <a href="{{url('admin/seasons/sync')}}" class="btn btn-success">Sync</a> -->
                <a href="{{url('admin/seasons/trashed')}}" class="btn btn-danger ms-2"ms-2>Trashed</a>
    ...
    <!-- Responsive Table -->
    <div class="card">
        <h5 class="card-header">Seasons</h5>
        <div class="card-body">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="d-flex justify-content-end">
                        <a class="btn btn-sm btn-primary" href="{{asset('assets/files/seasons-sample.csv')}}" download="seasons-sample.csv">Download Sample</a>
                    </div>
    ...
    $(document).ready(function() {
        $(document).ready(function() {
            ...
            $(document).on('click', '#check-all', function() {
                $('.season-checkbox').each(function() {
                    $(this).prop('checked', $(this).is(':checked')? false : true);
                    $('#delete-selected-seasons').addClass('disabled');
                    $('.season-checkbox:checked').each(function() {
                        $('#delete-selected-seasons').removeClass('disabled');
                    });
                });
            });

            $(document).on('click', '.season-checkbox', function() {
            ...
            $(document).on('click', '#delete-selected-seasons', function () {
                var selectedSeasons = [];
                $('.season-checkbox:checked').each(function() {
                    selectedSeasons.push($(this).attr('data-season-id'));
                });
                if (selectedSeasons.length > 0) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{route('delete_multiple_seasons')}}",
                                method: 'POST',
                                data: {
                                    _token: "{{csrf_token()}}",
                                    data_ids: selectedSeasons 
                                },
                    ...
                    Swal.fire(
                        'No season selected',
                        'Please select at least one season to delete.',
                        'info'
                    );
                }
            });
            ...
            $(document).on('click', '.delete-season', function () {
                var selectedSeason = $(this).attr('data-season-id');
                if (selectedSeason.length > 0) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{route('admin.seasons.destroy')}}",
                                method: 'POST',
                                data: {
                                    _token: "{{csrf_token()}}",
                                    data_id: selectedSeason 
                                },
                                success: function(response) {
                                    if (response.status) {
                                        Swal.fire(
