...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="row">
        <div class="col-12 d-flex justify-content-between">
            <div class="breadcrumb-list">
                <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Vehicle Types /</span> List</h4>
            </div>
            <div class="butns">
                <a href="{{url('admin/vehicle-types/create')}}" class="btn btn-primary ms-2">Add New</a>
                <!-- <a href="{{url('admin/vehicle-types/sync')}}" class="btn btn-success">Sync</a> -->
                <a href="{{url('admin/vehicle-types/trashed')}}" class="btn btn-danger ms-2"ms-2>Trashed</a>
    ...
    <!-- Responsive Table -->
    <div class="card">
        <h5 class="card-header">Vehicle Types</h5>
        <div class="card-body">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="d-flex justify-content-end">
                        <a class="btn btn-sm btn-primary" href="{{asset('assets/files/vehicle-types-sample.csv')}}" download="vehicle-types-sample.csv">Download Sample</a>
                    </div>
        ...
        $(document).ready(function() {
            ...
            if (!$.fn.dataTable.isDataTable(table)) {
                ...
                var table = table.DataTable({
                    ...
                    scrollX: true,
                    ajax: {
                        url: "{{url('admin/vehicle-types/list')}}",
                    },
                    columns: [{
                            name: 'sr_no',
                            data: 'sr_no'
            ...
            $(document).on('click', '#check-all', function() {
                $('.vehicle-type-checkbox').each(function() {
                    $(this).prop('checked', $(this).is(':checked')? false : true);
                    $('#delete-selected-vehicle-types').addClass('disabled');
                    $('.vehicle-type-checkbox:checked').each(function() {
                        $('#delete-selected-vehicle-types').removeClass('disabled');
                    });
                });
            });

            $(document).on('click', '.vehicle-type-checkbox', function() {
            ...
            $(document).on('click', '#delete-selected-vehicle-types', function () {
                var selectedVehicleTypes = [];
                $('.vehicle-type-checkbox:checked').each(function() {
                    selectedVehicleTypes.push($(this).attr('data-vehicle-type-id'));
                });
                if (selectedVehicleTypes.length > 0) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{route('admin.vehicle_types.list')}}",
                                method: 'POST',
                                data: {
                                    _token: "{{csrf_token()}}",
                                    data_ids: selectedVehicleTypes 
                                },
                                success: function(response) {
                                    if (response.status) {
                                        ...
                                        Swal.fire(
                                            'Failed!',
                                            'Failed to delete selected vehicle-types.',
                                            'error'
                                        );
                                    }
                                }
                    ...
                    Swal.fire(
                        ...
                        'Please select at least one vehicle-type to delete.',
                        'info'
                    );
                }
            });
            ...
            $(document).on('click', '.delete-vehicle-type', function () {
                var selectedVehicleType = $(this).attr('data-vehicle-type-id');
                if (selectedVehicleType.length > 0) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{route('admin.vehicle_types.destroy')}}",
                                method: 'POST',
                                data: {
                                    _token: "{{csrf_token()}}",
                                    data_id: selectedVehicleType 
                                },
                                success: function(response) {
                                    if (response.status) {
                                        Swal.fire(
