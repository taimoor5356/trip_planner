    ...
    <div class="row">
        <div class="col-12 d-flex justify-content-between">
            <div class="breadcrumb-list">
                <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Cities /</span> List</h4>
            </div>
            <div class="butns">
                <a href="{{url('admin/cities/create')}}" class="btn btn-primary ms-2">Add New</a>
                <!-- <a href="{{url('admin/cities/sync')}}" class="btn btn-success">Sync</a> -->
                <a href="{{url('admin/cities/trashed')}}" class="btn btn-danger ms-2"ms-2>Trashed</a>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="d-flex justify-content-end">
                        <a class="btn btn-sm btn-primary" href="{{asset('assets/files/cities-sample.csv')}}" download="cities-sample.csv">Download Sample</a>
                    </div>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '#check-all', function() {
                $('.city-checkbox').each(function() {
                    ...
                    $('#delete-selected-cities').addClass('disabled');
                    $('.city-checkbox:checked').each(function() {
                        $('#delete-selected-cities').removeClass('disabled');
                    });
                });
            });

            $(document).on('click', '.city-checkbox', function() {
            ...
            $(document).on('click', '#delete-selected-cities', function () {
                var selectedCities = [];
                $('.city-checkbox:checked').each(function() {
                    selectedCities.push($(this).attr('data-city-id'));
                });
                if (selectedCities.length > 0) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{route('delete_multiple_cities')}}",
                                method: 'POST',
                                data: {
                                    _token: "{{csrf_token()}}",
                                    data_ids: selectedCities 
                                },
                                success: function(response) {
                                    if (response.status) {
                                        ...
                                        Swal.fire(
                                            'Failed!',
                                            'Failed to delete selected cities.',
                                            'error'
                                        );
                                    }
                                }
                    ...
                    Swal.fire(
                        'No city selected',
                        'Please select at least one city to delete.',
                        'info'
                    );
                }
            });
            ...
            $(document).on('click', '.delete-city', function () {
                var selectedCity = $(this).attr('data-city-id');
                if (selectedCity.length > 0) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{route('admin.cities.destroy')}}",
                                method: 'POST',
                                data: {
                                    _token: "{{csrf_token()}}",
                                    data_id: selectedCity 
                                },
                                success: function(response) {
                                    if (response.status) {
                                        Swal.fire(
