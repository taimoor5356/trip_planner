...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="row">
        <div class="col-12 d-flex justify-content-between">
            <div class="breadcrumb-list">
                <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Land Marks /</span> List</h4>
            </div>
            <div class="butns">
                <a href="{{url('admin/land-marks/create')}}" class="btn btn-primary ms-2">Add New</a>
                <!-- <a href="{{url('admin/land-marks/sync')}}" class="btn btn-success">Sync</a> -->
                <a href="{{url('admin/land-marks/trashed')}}" class="btn btn-danger ms-2"ms-2>Trashed</a>
    ...
    <!-- Responsive Table -->
    <div class="card">
        <h5 class="card-header">Land Marks</h5>
        <div class="card-body">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="d-flex justify-content-end">
                        <a class="btn btn-sm btn-primary" href="{{asset('assets/files/land-marks-sample.csv')}}" download="land-marks-sample.csv">Download Sample</a>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '#check-all', function() {
            $('.land-mark-checkbox').each(function() {
                $(this).prop('checked', $(this).is(':checked')? false : true);
                $('#delete-selected-land-marks').addClass('disabled');
                $('.land-mark-checkbox:checked').each(function() {
                    $('#delete-selected-land-marks').removeClass('disabled');
                });
            });
        });

        $(document).on('click', '.land-mark-checkbox', function() {
        ...
        $(document).on('click', '#delete-selected-land-marks', function () {
            var selectedLandMarks = [];
            $('.land-mark-checkbox:checked').each(function() {
                selectedLandMarks.push($(this).attr('data-land-mark-id'));
            });
            if (selectedLandMarks.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{route('delete_multiple_land_marks')}}",
                            method: 'POST',
                            data: {
                                _token: "{{csrf_token()}}",
                                data_ids: selectedLandMarks 
                            },
                ...
                Swal.fire(
                    'No land-mark selected',
                    'Please select at least one land-mark to delete.',
                    'info'
                );
            }
        });

        $(document).on('click', '.delete-land-mark', function () {
            var selectedLandMark = $(this).attr('data-land-mark-id');
            if (selectedLandMark.length > 0) {
                Swal.fire({
                    title: 'Are you sure?',
                    title: 'Are you sure to DELETE?',
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{route('admin.land_marks.destroy')}}",
                            method: 'POST',
                            data: {
                                _token: "{{csrf_token()}}",
                                data_id: selectedLandMark 
                            },
                            success: function(response) {
                                if (response.status) {
                                    Swal.fire(
