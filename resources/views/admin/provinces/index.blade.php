    ...
    <div class="row">
        <div class="col-12 d-flex justify-content-between">
            <div class="breadcrumb-list">
                <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Provinces /</span> List</h4>
            </div>
            <div class="butns">
                <a href="{{url('admin/provinces/create')}}" class="btn btn-primary ms-2">Add New</a>
                <!-- <a href="{{url('admin/provinces/sync')}}" class="btn btn-success">Sync</a> -->
                <a href="{{url('admin/provinces/trashed')}}" class="btn btn-danger ms-2"ms-2>Trashed</a>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="d-flex justify-content-end">
                        <a class="btn btn-sm btn-primary" href="{{asset('assets/files/provinces-sample.csv')}}" download="provinces-sample.csv">Download Sample</a>
                    </div>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '#check-all', function() {
                $('.province-checkbox').each(function() {
                    ...
                    $('#delete-selected-provinces').addClass('disabled');
                    $('.province-checkbox:checked').each(function() {
                        $('#delete-selected-provinces').removeClass('disabled');
                    });
                });
            });

            $(document).on('click', '.province-checkbox', function() {
            ...
            $(document).on('click', '#delete-selected-provinces', function () {
                var selectedProvinces = [];
                $('.province-checkbox:checked').each(function() {
                    selectedProvinces.push($(this).attr('data-province-id'));
                });
                if (selectedProvinces.length > 0) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{route('delete_multiple_provinces')}}",
                                method: 'POST',
                                data: {
                                    _token: "{{csrf_token()}}",
                                    data_ids: selectedProvinces 
                                },
                                success: function(response) {
                                    if (response.status) {
                                        ...
                                        Swal.fire(
                                            'Failed!',
                                            'Failed to delete selected provinces.',
                                            'error'
                                        );
                                    }
                                }
                    ...
                    Swal.fire(
                        ...
                        'Please select at least one province to delete.',
                        'info'
                    );
                }
            });
            ...
            $(document).on('click', '.delete-province', function () {
                var selectedProvince = $(this).attr('data-province-id');
                if (selectedProvince.length > 0) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{route('admin.provinces.destroy')}}",
                                method: 'POST',
                                data: {
                                    _token: "{{csrf_token()}}",
                                    data_id: selectedProvince 
                                },
                                success: function(response) {
                                    if (response.status) {
                                        Swal.fire(
