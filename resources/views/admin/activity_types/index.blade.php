...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="row">
        <div class="col-12 d-flex justify-content-between">
            ...
            <div class="butns">
                <a href="{{url('admin/activity-types/create')}}" class="btn btn-primary ms-2">Add New</a>
                <!-- <a href="{{url('admin/activity-types/sync')}}" class="btn btn-success">Sync</a> -->
                <a href="{{url('admin/activity-types/trashed')}}" class="btn btn-danger ms-2"ms-2>Trashed</a>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="d-flex justify-content-end">
                        <a class="btn btn-sm btn-primary" href="{{asset('assets/files/activity-types-sample.csv')}}" download="activity-types-sample.csv">Download Sample</a>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '#check-all', function() {
            $('.activity-type-checkbox').each(function() {
                $(this).prop('checked', $(this).is(':checked')? false : true);
                $('#delete-selected-activity-types').addClass('disabled');
                $('.activity-type-checkbox:checked').each(function() {
                    $('#delete-selected-activity-types').removeClass('disabled');
                });
            });
        });

        $(document).on('click', '.activity-type-checkbox', function() {
        ...
        $(document).on('click', '#delete-selected-activity-types', function () {
            var selectedActivityTypes = [];
            $('.activity-type-checkbox:checked').each(function() {
                selectedActivityTypes.push($(this).attr('data-activity-type-id'));
            });
            if (selectedActivityTypes.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{route('delete_multiple_activity_types')}}",
                            method: 'POST',
                            data: {
                                _token: "{{csrf_token()}}",
                                data_ids: selectedActivityTypes 
                            },
                            success: function(response) {
                                if (response.status) {
                                    ...
                                    Swal.fire(
                                        'Failed!',
                                        'Failed to delete selected activity types.',
                                        'error'
                                    );
                                }
                            }
                ...
                Swal.fire(
                    ...
                    'Please select at least one activity-type to delete.',
                    'info'
                );
            }
        });

        $(document).on('click', '.delete-activity-type', function () {
            var selectedActivityType = $(this).attr('data-activity-type-id');
            if (selectedActivityType.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{route('admin.activity_types.destroy')}}",
                            method: 'POST',
                            data: {
                                _token: "{{csrf_token()}}",
                                data_id: selectedActivityType 
                            },
                            success: function(response) {
                                if (response.status) {
                                    Swal.fire(
