...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="row">
        <div class="col-12 d-flex justify-content-between">
            ...
            <div class="butns">
                <a href="{{url('admin/building-types/create')}}" class="btn btn-primary ms-2">Add New</a>
                <!-- <a href="{{url('admin/building-types/sync')}}" class="btn btn-success">Sync</a> -->
                <a href="{{url('admin/building-types/trashed')}}" class="btn btn-danger ms-2"ms-2>Trashed</a>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="d-flex justify-content-end">
                        <a class="btn btn-sm btn-primary" href="{{asset('assets/files/building-types-sample.csv')}}" download="building-types-sample.csv">Download Sample</a>
                    </div>
    ...
    $(document).ready(function() {
        ...
        if (!$.fn.dataTable.isDataTable(table)) {
            ...
            var table = table.DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: "{{url('admin/building-types/list')}}",
                },
                columns: [{
                        name: 'sr_no',
                        data: 'sr_no'
        ...
        $(document).on('click', '#check-all', function() {
            $('.building-types-checkbox').each(function() {
                $(this).prop('checked', $(this).is(':checked')? false : true);
                $('#delete-selected-building-types').addClass('disabled');
                $('.building-types-checkbox:checked').each(function() {
                    $('#delete-selected-building-types').removeClass('disabled');
                });
            });
        });

        $(document).on('click', '.building-types-checkbox', function() {
        ...
        $(document).on('click', '#delete-selected-building-types', function () {
            var selectedBuildingTypes = [];
            $('.building-types-checkbox:checked').each(function() {
                selectedBuildingTypes.push($(this).attr('data-building-type-id'));
            });
            if (selectedBuildingTypes.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{route('delete_multiple_building_types')}}",
                            method: 'POST',
                            data: {
                                _token: "{{csrf_token()}}",
                                data_ids: selectedBuildingTypes 
                            },
                            success: function(response) {
                                if (response.status) {
                                    ...
                                    Swal.fire(
                                        'Failed!',
                                        'Failed to delete selected building types.',
                                        'error'
                                    );
                                }
                            }
                ...
                Swal.fire(
                    ...
                    'Please select at least one building type to delete.',
                    'info'
                );
            }
        });

        $(document).on('click', '.delete-building-type', function () {
            var selectedBuildingType = $(this).attr('data-building-type-id');
            if (selectedBuildingType.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{route('admin.building_types.destroy')}}",
                            method: 'POST',
                            data: {
                                _token: "{{csrf_token()}}",
                                data_id: selectedBuildingType 
                            },
                            success: function(response) {
                                if (response.status) {
                                    Swal.fire(
