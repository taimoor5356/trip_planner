    ...
    <div class="row">
        <div class="col-12 d-flex justify-content-between">
            <div class="breadcrumb-list">
                <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Report /</span> Total Worked RVUs</h4>
            </div>
        </div>
    </div>
    <!-- Responsive Table -->
        ...
        <div class="card-body">
            <div class="table-responsive text-nowrap">
                <table class="table data-table display table-bordered responsive nowrap" width="100%">
                    ...
                    <tbody>
                        @php $total = 0; @endphp
                        @foreach($totalWorkedRvus as $key => $collection)
                            <tr>
                                <td>{{$key=='pendingRvus'?'Pending RVUs':($key=='paidRvus'?'Paid RVUs':'')}}</td>
                                <td class="text-center">{{$collection}}</td>
                            </tr>
                            @php $total += $collection; @endphp
                        @endforeach
                        <tr>
