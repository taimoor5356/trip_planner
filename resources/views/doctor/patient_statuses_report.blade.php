        ...
        <div class="card-body">
            <div class="table-responsive text-nowrap">
                <table class="table data-table display table-bordered responsive nowrap" width="100%">
                    <thead>
                        <th>Status</th>
                        <th class="text-center">No. of Pt</th>
                        <th class="text-center">Amount</th>
                    </thead>
                    <tbody>
                        @php $amount = 0; $pts = 0; @endphp
                        @foreach($patient_statuses as $key => $collection)
                            <tr>
                                <td>
                                    {{$key=='coInsurance'?'Co-Insurance Pending':($key=='coPayments'?'Co-Payment Pending':($key=='deductibles'?'Deductibles Pending':($key=='selfPay'?'Self Pay Pending':($key=='overPayment'?'Over Payment':''))))}}
                                    @foreach($collection as $key => $value)
                                        @if ($key == 'totalPatients')
                                        <td class="text-center">
                                            {{ ($value == '' || $value == 0) ? 0 : $value }}
                                        </td>
                                        @php $pts += (int)$value; @endphp
                                        @elseif ($key == 'totalAmount')
                                        <td class="text-center">
                                            ${{ ($value == '' || $value == 0) ? 0 : $value }}
                                        </td>
                                        @php $amount += (int)$value; @endphp
                                        @endif
                                    @endforeach
                                </td>
                            </tr>
