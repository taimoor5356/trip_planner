...
class TownController extends Controller
{
        ...
        return DataTables::of($data)
            ...
            ->addColumn('actions', function ($row) use ($trashed) {
                $btns = '
                    ...
                    if ($trashed == null) {
                        $btns .= '<a class="btn btns m-0 p-1" data-town-id="'.$row->id.'" href="edit/'.$row->id.'">
                                <i class="align-middle text-primary" data-feather="edit">
                                </i>
                            </a>
                            <a class="btn btns m-0 p-1 delete-town" data-town-id="'.$row->id.'" href="#">
    ...
    public function store(Request $request)
    {
        ...
        $town = new Town();
        $this->storeTown($town, $request);
        return redirect('admin/towns/list')->with('success', 'Town added successfully');
    }

    public function storeTown($town, $request) {
        $town->name = $request->name;
        $town->city_id = $request->city_id ?? null;
        $town->status = $request->status ?? 0;
        $town->save();
    }
    ...
    {
        //
        $data['header_title'] = 'Edit Town';
        $data['record'] = Town::find($id);
        $data['cities'] = City::where('status', 1)->get();
    ...
    public function update(Request $request, string $id)
    {
        ...
        $town = Town::find($id);
        $this->storeTown($town, $request);
        return redirect('admin/towns/list')->with('success', 'Town updated successfully');
    }
    ...
    {
        //
        $accommodation = Town::where('id', $request->data_id)->first();
