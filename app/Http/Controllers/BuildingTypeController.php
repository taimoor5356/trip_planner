...
{
    public $headerTitle = 'Building Type';
        ...
        return DataTables::of($data)
            ...
            ->addColumn('actions', function ($row) use ($trashed) {
                $btns = '
                    ...
                    if ($trashed == null) {
                        $btns .= '<a class="btn btns m-0 p-1" data-building-type-id="'.$row->id.'" href="edit/'.$row->id.'">
                                <i class="align-middle text-primary" data-feather="edit">
                                </i>
                            </a>
                            <a class="btn btns m-0 p-1 delete-building-type" data-building-type-id="'.$row->id.'" href="#">
    ...
    public function index(Request $request)
    {
        ...
        $data['header_title'] = 'Building Types';
        return view('admin.building_types.index', $data);
    }
    ...
    public function create()
    {
        ...
        return view('admin.building_types.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        $this->storeBuildingType($buildingType, $request);
        return redirect('admin/building-types/list')->with('success', 'Building Type added successfully');
    }

    public function storeBuildingType($buildingType, $request) {
        $buildingType->name = $request->name;
        $buildingType->status = $request->status ?? 0;
        $buildingType->save();
        Helpers::storeImage($request, \App\Models\BuildingImage::class, $buildingType, 'building_type_id', 'buildings');
    }
    ...
    {
        //
        $data['header_title'] = 'Edit Building Type';
        $data['record'] = BuildingType::find($id);
        return view('admin.building_types.edit', $data);
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $this->storeBuildingType($buildingType, $request);
        return redirect('admin/building-types/list')->with('success', 'Building Type updated successfully');
    }
    ...
    {
        //
        $accommodation = BuildingType::where('id', $request->data_id)->first();
