...
class RegionController extends Controller
{
    ...
    public function datatables($request, $records, $trashed = null)
    {
        ...
        return DataTables::of($data)
            ...
            ->addColumn('province_name', function ($row) {
                return ucwords($row->province?->name);
            })
            ...
            ->addColumn('actions', function ($row) use ($trashed) {
                $btns = '
                    ...
                    if ($trashed == null) {
                        $btns .= '<a class="btn btns m-0 p-1" data-region-id="'.$row->id.'" href="edit/'.$row->id.'">
                                <i class="align-middle text-primary" data-feather="edit">
                                </i>
                            </a>
                            <a class="btn btns m-0 p-1 delete-region" data-region-id="'.$row->id.'" href="#">
    ...
    public function index(Request $request)
    {
        ...
        $data['header_title'] = 'Regions List';
    ...
    public function store(Request $request)
    {
        ...
        $region = new Region();
        $this->storeRegion($region, $request);
        return redirect('admin/regions/list')->with('success', 'Region added successfully');
    }

    public function storeRegion($region, $request) {
        $region->name = $request->name;
        $region->province_id = $request->province_id ?? null;
        $region->status = $request->status ?? 0;
        $region->save();
        Helpers::storeImage($request, \App\Models\RegionImage::class, $region, 'region_id', 'regions');
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $this->storeRegion($region, $request);
        return redirect('admin/regions/list')->with('success', 'Region updated successfully');
    }
    ...
    {
        //
        $accommodation = Region::where('id', $request->data_id)->first();
